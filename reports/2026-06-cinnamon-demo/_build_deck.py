...
def add_title_bar(slide, title, subtitle=None):
    ...
    run = p.add_run()
    run.text = title
    set_text(run, size=28, bold=True, color=WHITE)
    if subtitle:
        p2 = tf.add_paragraph()
        p2.alignment = PP_ALIGN.LEFT
        run2 = p2.add_run()
        run2.text = subtitle
        set_text(run2, size=14, color=STONE_100)
...
def add_footer(slide, text="ForestMRV v2.11.69 │ 土肉桂專區葉片收穫監測會計系統"):
    ...
    run = p.add_run()
    run.text = text
    set_text(run, size=9, color=STONE_500)
...
def add_section_divider(prs, section_no, section_title, time_range=None):
    ...
    bg.fill.solid()
    bg.fill.fore_color.rgb = FOREST_800
    ...
    set_text(run, size=36, color=AMBER_500, bold=True)

    # 段標題
    tb2 = slide.shapes.add_textbox(Inches(1.5), Inches(3.3), Inches(10.3), Inches(2))
    tf2 = tb2.text_frame
    p2 = tf2.paragraphs[0]
    p2.alignment = PP_ALIGN.CENTER
    run2 = p2.add_run()
    run2.text = section_title
    set_text(run2, size=44, color=WHITE, bold=True)

    if time_range:
        tb3 = slide.shapes.add_textbox(Inches(1.5), Inches(5.5), Inches(10.3), Inches(0.8))
        tf3 = tb3.text_frame
        p3 = tf3.paragraphs[0]
        p3.alignment = PP_ALIGN.CENTER
        run3 = p3.add_run()
        run3.text = time_range
        set_text(run3, size=20, color=STONE_300)
...
def add_content_slide(prs, title, bullets=None, image=None, image_caption=None,
    ...
    tb = slide.shapes.add_textbox(Inches(0.5), Inches(1.4), text_width, Inches(5.5))
    tf = tb.text_frame
    ...
    if bullets:
        for i, b in enumerate(bullets):
            ...
            if isinstance(b, tuple):
                ...
                run = p.add_run()
                run.text = heading
                set_text(run, size=18, bold=True, color=FOREST_700)
                if body:
                    run2 = p.add_run()
                    run2.text = " " + body
                    set_text(run2, size=15, color=STONE_700)
            elif b.startswith("# "):
                run = p.add_run()
                run.text = b[2:]
                set_text(run, size=20, bold=True, color=FOREST_700)
            elif b.startswith("> "):
                p.level = 1
                run = p.add_run()
                run.text = " " + b[2:]
                set_text(run, size=14, italic=True, color=STONE_500)
            else:
                run = p.add_run()
                run.text = "• " + b
                set_text(run, size=15, color=STONE_700)

    if image:
        img_path = IMG / image
        if img_path.exists():
            slide.shapes.add_picture(str(img_path), Inches(image_left), Inches(image_top),
                                     width=Inches(image_width))
            if image_caption:
                cap_tb = slide.shapes.add_textbox(
                    Inches(image_left), Inches(image_top + image_width * 0.7 + 0.1),
                ...
                set_text(crun, size=10, italic=True, color=STONE_500)
        else:
            # 圖檔缺失提示
...
def add_big_image_slide(prs, title, image, image_caption=None, image_width=10.0):
    ...
    if img_path.exists():
        ...
        img_w_emu = Inches(image_width)
        left = (slide_w - img_w_emu) // 2
        slide.shapes.add_picture(str(img_path), left, Inches(1.3), width=img_w_emu)
        if image_caption:
            cap_tb = slide.shapes.add_textbox(Inches(1), Inches(6.6),
            ...
            crun = cp.add_run()
            crun.text = image_caption
            set_text(crun, size=11, italic=True, color=STONE_500)
    else:
...
bg = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, 0, 0, prs.slide_width, prs.slide_height)
bg.fill.solid()
bg.fill.fore_color.rgb = FOREST_900
bg.line.fill.background()

# 主標題
tb = slide.shapes.add_textbox(Inches(0.5), Inches(1.8), Inches(12.3), Inches(2.0))
tf = tb.text_frame
p = tf.paragraphs[0]
p.alignment = PP_ALIGN.CENTER
run = p.add_run()
run.text = "土肉桂專區"
set_text(run, size=54, bold=True, color=AMBER_500)

p2 = tf.add_paragraph()
p2.alignment = PP_ALIGN.CENTER
run2 = p2.add_run()
run2.text = "葉片收穫監測會計系統"
set_text(run2, size=44, bold=True, color=WHITE)

# 副標題
tb_sub = slide.shapes.add_textbox(Inches(0.5), Inches(4.5), Inches(12.3), Inches(1.0))
tf_sub = tb_sub.text_frame
p_sub = tf_sub.paragraphs[0]
p_sub.alignment = PP_ALIGN.CENTER
run_sub = p_sub.add_run()
run_sub.text = "系統說明會"
set_text(run_sub, size=28, color=STONE_100)

# 底部資訊
tb_info = slide.shapes.add_textbox(Inches(0.5), Inches(6.2), Inches(12.3), Inches(0.8))
...
set_text(run_info, size=16, color=STONE_300)
...
# =====================================================
add_content_slide(prs, "法定許可文號自動配發",
    bullets=[
        "# 機制",
        "Firestore runTransaction 原子遞增",
        "確保連號、不可重號、不可並寫",
        "# 文號格式",
        "「林保中-土肉桂修枝-{民國年}-{NNN}」",
        "例：林保中-土肉桂修枝-115-001",
        "# 同步動作",
        "申請狀態 submitted → approved",
        "寫入效期、審核附註",
        "Toast 通知：「✅ 已核准，許可文號 …」",
        "案件移出待審核區、進入已處理區",
        "# 林農端即時可見",
...
# Slide 26: 結案閘門
# =====================================================
add_content_slide(prs, "回報完畢並結案",
    bullets=[
        "# 結案動作",
        "點案件卡片「✅ 回報完畢並結案」",
        "確認對話框列出許可文號、累計回報量",
        "結案後狀態 harvesting → completed（已結案）",
        "# 結案後限制",
        "不能再填報採收量",
        "不能再編輯案件",
        "資料固定供分署查核與合作社彙整",
        "# 結案閘門（v2.11.39 ship）",
        ("法定義務", "totalLogged_kg > 0 才能結案"),
        ("雙重把關", "Firestore Rules + 客戶端同步擋"),
        "零回報直接結案會被擋下並提示先回報",
    ])
...
# Slide 29: 共同銷售規劃
# =====================================================
add_content_slide(prs, "共同銷售規劃",
    bullets=[
        "# 區塊一：專區總覽",
        "申請案總數、狀態分布、已回報葉片採收量總計",
        "共同銷售規劃主要依據",
        "# 區塊二：依林農葉片採收彙整",
        "三欄結構：林農 / 案件數 / 已回報葉片採收量(kg)",
        "依採收量降序排列",
        ("v2.11.41 拍板", "無達成率欄、純累計顯示"),
        "# 區塊三：各申請案（唯讀）",
        "可預覽每筆採取許可單",
        "# 匯出 Excel",
        "供共同銷售前資料整理、通路議價、結算分潤",
    ])

# =====================================================
# Slide 30: 五個角色重點回顧
# =====================================================
add_content_slide(prs, "結語：五個角色的協作鏈",
    bullets=[
        "# 行政管理鏈",
        "1. admin 開案、設邊界、邀請成員",
        "2. surveyor（林農）提出修枝申請、線上送出 + 紙本公文",
        "3. harvest_authority（分署）審核、核准、配發法定許可文號",
        "4. surveyor 線下修枝採葉、線上回報採收量、結案",
        "5. coop（合作社）唯讀彙整、共同銷售規劃、匯出 Excel",
        "",
        "# v2.11.69 業務拍板回顧",
...
tb = slide.shapes.add_textbox(Inches(0.5), Inches(2.5), Inches(12.3), Inches(2.5))
tf = tb.text_frame
p = tf.paragraphs[0]
p.alignment = PP_ALIGN.CENTER
run = p.add_run()
run.text = "Q & A"
set_text(run, size=80, bold=True, color=AMBER_500)
...
run2 = p2.add_run()
run2.text = "歡迎提問"
set_text(run2, size=32, color=WHITE)

tb_info = slide.shapes.add_textbox(Inches(0.5), Inches(6.0), Inches(12.3), Inches(0.8))
tf_info = tb_info.text_frame
p_info = tf_info.paragraphs[0]
p_info.alignment = PP_ALIGN.CENTER
run_info = p_info.add_run()
run_info.text = "土肉桂專區葉片收穫監測會計系統 │ ForestMRV v2.11.69"
set_text(run_info, size=14, color=STONE_300)
