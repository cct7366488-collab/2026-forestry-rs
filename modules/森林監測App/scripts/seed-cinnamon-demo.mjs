...
//   node seed-cinnamon-demo.mjs --clean    # 清除所有 demoSeed 案與其 logs
//
  ...
  if (CLEAN) {
    ...
    for (const id of existing) {
      ...
      try {
        ...
        for (const l of (ls.documents || [])) writes.push(del(`${HP}/${id}/logs/${l.name.split('/').pop()}`));
      } catch {}
      writes.push(del(`${HP}/${id}`));
    }
    if (writes.length) await commit(token, writes);
    console.log(`\n🧹 已清除 ${existing.length} 個 demo 案（含其收穫紀錄）。`);
    return;
  }
