...
import { fb, $, $$, el, toast, openModal, closeModal, state, calcTreeMetrics, speciesParamsLabel, wgs84ToTwd97, twd97ToWgs84, DEFAULT_METHODOLOGY, isPi, isDataManager, isSurveyor, isReviewer, isSystemAdmin, canQA, isLocked, rerouteCurrentView, captureCurrentSubtab, qaBadge } from './app.js?v=21157';
// v2.7.16：樣區幾何 + 坡度修正 utility
import { computeAreaHorizontal, computeAreaHorizontal2D, computeAreaSlope, computeAreaSlope2D, nominalToSlopeDistance, dimensionsToArea } from './plot-geometry.js?v=21157';
// v2.7.17：reviewer QAQC 工作流
// v2.8.1：tree-level QAQC（抽樣 / 重測 / 誤差 / 處置）
import { DEFAULT_QAQC_CONFIG, defaultQaqc, defaultTreeQaqc, computeQaqcErrors, computeTreeQaqcErrors, computeTreeSampleSize, pickRandomTreeSample, getTreeQaqcStatus, RESOLUTION_LABEL } from './plot-qaqc.js?v=21157';
// v2.8.0：irregular plot 不規則多邊形（Shoelace / 自交檢查 / GeoJSON 解析）
import { validatePolygon, parseGeoJsonPolygon, parseProjectBoundaryGeoJson, shoelaceArea, computeBbox, vertsToArrays, arraysToVerts, VERTEX_MIN, VERTEX_MAX } from './plot-polygon.js?v=21157';
import { TYPE_CODES, AGENCY_CODES, agenciesByGroup, nextSequence, buildProjectCode } from './code-tables.js?v=21157';
// 每專案模組組合（軸 A/B）：新專案依計畫類型帶套餐預設、可勾選微調
import { MODULES, FAMILIES, defaultModulesForType, getModule } from './module-registry.js?v=21157';
// v2.0：物種字典從 species-dict.js 載入（樹種 / 動物 / 草本 / 入侵種）
import { TREES, ANIMALS, HERBS, INVASIVE_PLANTS, isInvasive, findHerb, findAnimal } from './species-dict.js?v=21157';
// v2.10.5：樹種搜尋下拉組件（取代 <datalist>，支援 Firestore 224 種 + fuzzy match）
import { createSpeciesPicker } from './species-picker.js?v=21157';
// v2.10.9：DEM 海拔自動偵測（plot GPS → 海拔 → picker band）
import { getElevation, elevationToBand, bandLabel } from './dem-elevation.js?v=21157';
// v2.11.0：AI 樹種辨識 modal（Pl@ntNet 線上 API）
import { openAiIdentifyModal } from './ai-identify-modal.js?v=21157';
// v2.3：階段 2 狀態機（自動偵測送審）
import { STATUS, applyStatusAfterQA, applyStatusAfterSurveyorReset, applyStatusAfterMethodologySaved } from './project-status.js?v=21157';
...
export async function openPlotForm(project, existing = null) {
  ...
  const gpsHelpDetails = el('details', {
    ...
    el('div', { class: 'mt-2' },
      el('a', {
        href: './img/gps-position-guide.svg?v=21157',
        target: '_blank',
        rel: 'noopener',
        class: 'block',
        ...
        el('img', {
          src: './img/gps-position-guide.svg?v=21157',
