...
import { firebaseConfig } from "../firebase-config.js?v=21195";
import * as forms from "./forms.js?v=21195";
import * as analytics from "./analytics.js?v=21195";
import * as importWizard from "./import-wizard.js?v=21195";
// v2.11.33：土肉桂葉片採收許可電子化（林農申請 → 林保署核准）
import * as harvestPermits from "./harvest-permits.js?v=21195";
import { renderTreeDistribution } from "./distribution.js?v=21195";   // v2.6.2：立木分布散布圖
import { initTreeMap } from "./tree-map.js?v=21195";                    // v2.11.29：plot detail Leaflet 地圖
import { renderSpeciesDict, disposeSpeciesDict } from "./species-admin.js?v=21195";   // v2.7.10：admin 樹種字典管理
// v2.7.17：reviewer QAQC 工作流
// v2.8.1：tree-level QAQC（抽樣 / 重測 / 誤差 / 處置 / gate）
import { DEFAULT_QAQC_CONFIG, computeTargetSampleSize, computeTreeSampleSize, pickRandomSample, getPlotQaqcStatus, getTreeQaqcStatus, QAQC_STATUS_META, RESOLUTION_LABEL, checkApprovalGate, checkTreeApprovalGate, computeErrorStats, computeTreeErrorStats, defaultQaqc, defaultTreeQaqc } from "./plot-qaqc.js?v=21195";
import { calcTreeMetrics as calcTreeMetricsImpl, speciesParamsLabel as speciesParamsLabelImpl, getEquationBadge } from "./species-equations.js?v=21195";
// 每專案模組開關（軸 A）+ 軸 B：依計畫類型/調查需求 gating 分頁與子調查
import { MODULES, moduleEnabled, tabEnabled, subtabEnabled } from "./module-registry.js?v=21195";
// v2.3：階段 2 — 狀態機 + 自動偵測送審；v2.7：階段 3 — Reviewer 完成審查
import { STATUS, STATUS_META, AUTO_LOCK_REASON_LABEL, statusBadgeHTML, ensureStatusMigrated, applyStatusAfterManualLock, applyStatusAfterReviewerApprove, applyStatusRevertVerified, applyStatusForceUnlockReview, computeProgress } from "./project-status.js?v=21195";
...
export function el(tag, attrs = {}, ...children) {
  ...
  for (const c of children.flat()) {
    if (c == null) continue;
    // v2.10.1：number / boolean 也轉 textNode（之前只接 string，遇 number 會 appendChild throw）
...
async function triggerRectConversion(projectId) {
  ...
  try {
    const m = await import('./migration-v2715.js?v=21195');
    toast('掃描中...');
...
if ('serviceWorker' in navigator) {
  ...
  navigator.serviceWorker.register('./service-worker.js', { updateViaCache: 'none' }).then(reg => {
    // page load 當下若已有 waiting SW（前次 tab 留下），立即顯示橫幅
...
function showUpdateBanner(reg) {
  ...
  if (!banner) return;
  banner.classList.remove('hidden');
  const btn = banner.querySelector('[data-update-apply]');
  if (btn && !btn._bound) {
    btn._bound = true;  // 避免多次 updatefound / waiting 觸發重綁
...
async function renderProjectHome(root, projectId) {
  ...
  $$('.tab-link').forEach(a => a.addEventListener('click', e => {
    ...
    if (tab === 'harvestreport') harvestPermits.renderHarvestReport(state.project); // v2.11.39：林農端採收回報及結案
  ...
  // v2.11.70（I-6）：複查成長報表 — on-demand（重讀逐期 measurement 歷史）
  $('#btn-resurvey-report')?.addEventListener('click', () => analytics.renderResurveyReport(state.project));
  $('#btn-resurvey-csv')?.addEventListener('click', () => analytics.exportResurveyCsv());
  ...
  // v2.11.82：專案層批次複查期別（PI/admin；權限與資料安全由 forms 內把關）
  $('#btn-open-all-periods')?.addEventListener('click', () => forms.openNewPeriodAllPlots(state.project));
  $('#btn-undo-all-periods')?.addEventListener('click', () => forms.undoLatestPeriodAllPlots(state.project));
  ...
  // v2.11.78：只先取 uid；label 解析移到 handler 內（await prefetchUserLabels 後），
  //   避免名稱快取尚未就緒（loadProject 的 prefetch 是 fire-and-forget）→ 指派清單顯示 UID 片段。
  const surveyorUids = Object.entries(state.project.members || {})
  ...
  const unsub = onSnapshot(qPlots, async snap => {
    ...
    // I-6c（v2.11.87）：複查樣區「本期複查進度」— 沿用同一圈 tree fetch（零額外讀取）算 measured/total
    const resurveyByPlot = new Map();  // plotId → { total, measured, curSeq }
    try {
      const statsList = await Promise.all(
        allDocs.flatMap(d => {
          const pdata = d.data();
          const pMulti = plotHasMultiPeriod(pdata);
          const pCurSeq = Number(pdata.currentPeriod) || 1;
          return enabledColls.map(async coll => {
            const ref = fb.collection(fb.db, 'projects', projectId, 'plots', d.id, coll);
            const csnap = await fb.getDocs(ref);
            let total = 0, verified = 0, measured = 0;
            csnap.forEach(td => {
              total++;
              const tdd = td.data();
              if (tdd.qaStatus === 'verified') verified++;
              // 只在複查樣區（≥2 期）的 trees 子集合算「本期已複查」
              if (coll === 'trees' && pMulti && treeMeasuredInPeriod(tdd, pCurSeq, pMulti)) measured++;
            });
            return { plotId: d.id, coll, total, verified, measured, multi: pMulti, curSeq: pCurSeq };
          });
        })
      );
      statsList.forEach(s => {
        ...
        if (s.coll === 'trees' && s.multi) {
          resurveyByPlot.set(s.plotId, { total: s.total, measured: s.measured, curSeq: s.curSeq });
        }
      });
    } catch (e) { console.warn('[v2.7.2 subcoll qa stats]', e); }
    // v2.11.78：確保指派 checkbox / 卡片標籤拿到 displayName/email（race 修：loadProject 的 prefetch 可能尚未就緒）
    await prefetchUserLabels(surveyorUids);
    // 較新的 snapshot 已進來（cache→server 或快速連續更新）→ 放棄這次（舊）render，避免重複 append
    if (mySeq !== _plotListSeq) return;
    list.innerHTML = '';
    // v2.11.78：name 快取就緒後才解析指派清單 label（避免顯示 UID 片段）
    const surveyors = surveyorUids.map(uid => ({ uid, label: userLabel(uid, uid.slice(0, 6)) }));
    ...
    visible.forEach(d => {
      ...
      if (rs && rs.total > 0) {
        ...
        headerRight.appendChild(el('span', {
          ...
          title: `第 ${rs.curSeq} 期複查進度：已複查 ${rs.measured} / 共 ${rs.total} 株${done ? '（本樣區本期已完成）' : '（尚缺 ' + (rs.total - rs.measured) + ' 株）'}`
        }, done ? `✅ 已複查 ${rs.measured}/${rs.total}` : `⏳ 待複查 ${rs.measured}/${rs.total}`));
      }
  ...
  // v2.11.84：複查（逐期）匯出集中到匯出分頁 — 原始值 Excel（B）＋成長摘要 CSV（standalone）
  $('#btn-export-resurvey-raw')?.addEventListener('click', () => analytics.exportPerTreeMeasurements(state.project));
  $('#btn-export-resurvey-summary')?.addEventListener('click', () => analytics.exportResurveyCsvStandalone(state.project));
}
...
async function renderPlotDetail(root, projectId, plotId) {
  ...
  bindData(root, 'plot', {
    ...
    // v2.11.94：不再讀 plot.insideBoundary（寫入端一律硬編 true，永遠顯示 ✅）—
    //   改為即時對專案邊界做點面套疊，並區分「界外」與「尚未上傳邊界」。
    insideBoundary: ({
      inside: '✅ 是',
      outside: '⚠ 座標在專案邊界外',
      'no-boundary': '— 尚未上傳專案邊界，無從比對',
      'no-coords': '— 樣區無座標',
    })[analytics.getPlotBoundaryStatus(state.plot, state.project)],
  ...
  // ===== I-2（路 I 複查）：期別 banner — 顯示當期 + 歷期 + 「📅 開啟新一期複查」入口 =====
  const periodBanner = $('#plot-period-banner');
  if (periodBanner) {
    const periods = forms.derivePlotPeriods(state.plot);
    ...
    periodBanner.appendChild(el('div', { class: 'flex justify-between items-center gap-2 flex-wrap' },
      el('div', { class: 'text-sm' },
        el('span', { class: 'font-semibold text-blue-900' }, '📅 當期：'),
        el('span', { class: 'text-blue-900' }, cur.label),
      ...
      el('div', { class: 'flex gap-2 flex-shrink-0 flex-wrap' },
        ...
        // v2.11.82：撤銷最新一期（開錯期的安全網）— 僅 PI/admin、且當期 ≥2（有複查期可撤）
        (canOpenPeriod && Number(cur.seq) >= 2) ? el('button', {
          class: 'bg-white hover:bg-red-50 text-red-700 border border-red-300 px-3 py-1.5 rounded text-sm shadow-sm',
          onclick: () => forms.undoLatestPeriod(state.project, state.plot)
        }, '↩️ 撤銷最新一期') : null,
    ...
    if (periods.length > 1) {
      periodBanner.appendChild(el('details', { class: 'mt-2 text-xs text-blue-800' },
        el('summary', { class: 'cursor-pointer select-none' }, `歷期紀錄（${periods.length} 期）`),
        el('ul', { class: 'mt-1 space-y-0.5 list-disc list-inside' },
          ...periods.slice().sort((a, b) => a.seq - b.seq).map(p =>
            el('li', {}, `${p.label}：${fmtP(p.openedAt)} ～ ${p.closedAt ? fmtP(p.closedAt) : '進行中'}${p.note ? '（' + p.note + '）' : ''}`)
          )
        )
      ));
    }
  ...
  // I-3（v2.11.70）：樹牌號碼跳轉 — 查當前 onSnapshot 的 _lastTreesArr（即時、零額外 fetch）
  ...
  const mods = state.project.methodology?.modules || {};   // 下方子集合訂閱（understory/soilCons/wildlife/harvest）仍引用
...
// I-6c（v2.11.86/87）：複查「本期是否已複查」共用判斷 — 立木清單紅/黑字 + 樣區清單進度共用，兩處邏輯必須一致
function plotHasMultiPeriod(plot) {
  return (Array.isArray(plot?.periods) ? plot.periods.length : (Number(plot?.currentPeriod) || 1)) >= 2;
}
function treeMeasuredInPeriod(t, curSeq, hasMulti) {
  if (!hasMulti) return true;                          // 第一期：無「上期/本期」之分
  if (Number(t.lastMeasuredPeriod) === curSeq) return true;  // 主判斷：本期存檔蓋的期號
  // 回退（本功能上線前、本期已重測但無 lastMeasuredPeriod 的樹）：updatedAt 晚於開期蓋印時間
  const toMs = (x) => x?.toMillis ? x.toMillis() : (typeof x?.seconds === 'number' ? x.seconds * 1000 : null);
  const s = toMs(t.priorSnapshot?.stampedAt), u = toMs(t.updatedAt);
...
function renderTreeList(snap, projectId, plotId) {
  ...
  const hasMultiPeriod = plotHasMultiPeriod(state.plot);
  // I-6c（v2.11.86）：複查期別「本期已複查」判斷 — 未複查者 DBH/H 顯示紅字（＝仍是上期資料）
  const curSeq = Number(state.plot?.currentPeriod) || 1;
  let pendingCount = 0;
  const rows = snap.docs.map(d => {
    const t = d.data();
    const measured = treeMeasuredInPeriod(t, curSeq, hasMultiPeriod);
    ...
    // v2.10.8（backlog #13）：公式來源徽章 — hover 顯示文獻 source
    const eqInfo = getEquationBadge(t.speciesZh, t.speciesSci, t.treeType);
    const eqTitle = `${eqInfo.level} | ${eqInfo.key} | BEF ${eqInfo.bef} CF ${eqInfo.cf} | ${eqInfo.source}`.replace(/"/g, '&quot;');
    const eqBadgeHtml = `<span title="${eqTitle}" style="cursor:help">${eqInfo.badge}</span>`;
    // I-5（v2.11.70）：複查狀態小徽章 — recruitment ✨ / 本期死亡 ☠ / 失蹤 ❓ / 樹牌脫落 🏷️
    let resBadge = '';
    if (t.recruitedPeriod && Number(t.recruitedPeriod) > 1) resBadge += ` <span title="第 ${t.recruitedPeriod} 期新進個體（recruitment）" style="font-size:11px;color:#047857">✨</span>`;
    if (t.resurveyFate === 'dead') resBadge += ' <span title="本期死亡" style="font-size:11px;color:#b91c1c">☠</span>';
    else if (t.resurveyFate === 'missing') resBadge += ' <span title="失蹤·無法尋獲" style="font-size:11px;color:#b45309">❓</span>';
    else if (t.resurveyFate === 'tag-lost') resBadge += ' <span title="樹牌脫落·身分存疑" style="font-size:11px;color:#b45309">🏷️</span>';
    const speciesCell = el('td', { html: eqBadgeHtml + ' ' + t.speciesZh + (t.conservationGrade ? ' ⚠' : '') + photoTag + resBadge + ' ' + qaBadge(t.qaStatus) });
    if (canQA() && !isLocked()) {
...
function renderUnderstoryList(snap, projectId, plotId) {
  ...
  const rows = snap.docs.map(d => {
    ...
    // v2.11.97：逐物種鑑定照的張數另計 —— 與樣方俯拍照分開顯示，才看得出「物種有沒有附鑑定照」
    const spPhotoCount = (u.species || []).reduce((s, sp) => s + ((sp.photos || []).length), 0);
    const spPhotoTag = spPhotoCount > 0
      ? ` <span style="font-size:11px;color:#15803d" title="物種鑑定照">🌿📷${spPhotoCount}</span>` : '';
    const stationLabel = { N: '北', E: '東', S: '南', W: '西', C: '中' }[u.quadratCode] || u.quadratCode;
    const speciesCell = el('td', { html: `${stationLabel} (${u.quadratSize})${photoTag}${spPhotoTag} ${qaBadge(u.qaStatus)}` });
    if (canQA() && !isLocked()) {
