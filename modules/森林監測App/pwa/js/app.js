...
import { firebaseConfig } from "../firebase-config.js?v=21159";
import * as forms from "./forms.js?v=21159";
import * as analytics from "./analytics.js?v=21159";
import * as importWizard from "./import-wizard.js?v=21159";
// v2.11.33：土肉桂葉片採收許可電子化（林農申請 → 林保署核准）
import * as harvestPermits from "./harvest-permits.js?v=21159";
import { renderTreeDistribution } from "./distribution.js?v=21159";   // v2.6.2：立木分布散布圖
import { initTreeMap } from "./tree-map.js?v=21159";                    // v2.11.29：plot detail Leaflet 地圖
import { renderSpeciesDict, disposeSpeciesDict } from "./species-admin.js?v=21159";   // v2.7.10：admin 樹種字典管理
// v2.7.17：reviewer QAQC 工作流
// v2.8.1：tree-level QAQC（抽樣 / 重測 / 誤差 / 處置 / gate）
import { DEFAULT_QAQC_CONFIG, computeTargetSampleSize, computeTreeSampleSize, pickRandomSample, getPlotQaqcStatus, getTreeQaqcStatus, QAQC_STATUS_META, RESOLUTION_LABEL, checkApprovalGate, checkTreeApprovalGate, computeErrorStats, computeTreeErrorStats, defaultQaqc, defaultTreeQaqc } from "./plot-qaqc.js?v=21159";
import { calcTreeMetrics as calcTreeMetricsImpl, speciesParamsLabel as speciesParamsLabelImpl, getEquationBadge } from "./species-equations.js?v=21159";
// 每專案模組開關（軸 A）+ 軸 B：依計畫類型/調查需求 gating 分頁與子調查
import { MODULES, moduleEnabled, tabEnabled, subtabEnabled } from "./module-registry.js?v=21159";
// v2.3：階段 2 — 狀態機 + 自動偵測送審；v2.7：階段 3 — Reviewer 完成審查
import { STATUS, STATUS_META, AUTO_LOCK_REASON_LABEL, statusBadgeHTML, ensureStatusMigrated, applyStatusAfterManualLock, applyStatusAfterReviewerApprove, applyStatusRevertVerified, applyStatusForceUnlockReview, computeProgress } from "./project-status.js?v=21159";
...
async function triggerRectConversion(projectId) {
  ...
  try {
    const m = await import('./migration-v2715.js?v=21159');
    toast('掃描中...');
