...
//   ?v=21145 -> ?v=21146 全檔。
const CACHE = 'forest-monitor-v2.11.97';  // v2.11.97（地被逐物種鑑定照）：地被樣方一次記到 20 種時，各物種的鑑定照原本只能全塞進「樣方俯拍照片」，查證時對不回哪張是哪一種（MRV 可回溯性缺口）。改為物種紀錄每一列各自掛照片：schema 於 species[] 每個元素加 photos[]（陣列→物件→陣列，非 Firestore 禁止的陣列直接巢狀，已實測寫入讀回確認）；表單每列一個上傳元件、以「列物件本身」為 key 存放（不可用索引：刪列是 splice，索引整批位移會讓剩下的列接到別人的照片），重繪時沿用同一實例故待上傳預覽不會被清掉；存檔須先建文件取得 docId 才能上傳（Storage 路徑含 docId），故走「先寫 species → 逐列 commit → 補寫回 species」。俯拍照欄位標籤改寫清楚兩者分工。清單加 🌿📷N 徽章與俯拍照 📷N 分開計數；匯出「地被植物」工作表每物種列加「鑑定照張數／鑑定照網址」。順帶修：物種名稱輸入框原本每一次按鍵都整列重繪、當場失去焦點，改為僅在入侵種標示改變時才重繪。?v= 21194→21195。以下歷史 // v2.11.95（照片上傳：相簿多選＋本地壓縮＋並行上傳）：地被一個小樣方記到 20 種就想放 20 張照片 — 查核後張數本來就沒上限（photoUploader／firestore.rules／storage.rules 三處皆無數量檢查，只限單張 5MB），真正卡住的是 file input 帶 capture=environment：手機瀏覽器見到 capture 會直接開相機並忽略同一個 input 上的 multiple，相簿多選整個沒得用，只能按 20 次逐張拍。改為兩個入口各司其職：＋方塊＝拍照（保留 capture、現場拍立傳手感不變），另加「🖼️ 從相簿選（可多選）」用不帶 capture 的 input。另補兩項：①新增零相依模組 js/image-compress.js 做上傳前本地壓縮（長邊 1600px／JPEG 0.85，只處理 >1.2MB 者；先壓再檢查 5MB 上限，手機原檔常本來就超標被 storage.rules 擋掉只留一句 toast）— 小檔不動避免二次失真、壓完變大用原檔、任何一步失敗一律退回原檔，絕不因壓縮失敗擋住調查員；⚠ 原用 img.decode() 等解碼，實測有環境圖片明明已載入（naturalWidth 有值）promise 卻永不 resolve → 表單永遠停在「處理照片中…」，改 onload/onerror＋8 秒逾時。②commit() 由序列改限流並行（3 條；野外上行頻寬有限，20 條同時搶只會互相拖慢又更易整批 timeout）＋進度回報，送出鈕顯示「上傳照片中… N/20」、元件下方顯示「處理照片中…／上傳中…」，避免一次 20 張時畫面不動被當成當掉；並行完成順序不固定故結果寫回原索引以保持與畫面排列一致。quickAddPhoto 同步套用壓縮。新增 tests/browser-image-compress.html（17/17，真瀏覽器）。SHELL 加入 image-compress.js。?v= 21193→21194。以下歷史 // v2.11.94（匯出「在範圍內」欄位失真修正）：樣區匯出原本輸出 plot.insideBoundary，但該欄位四個寫入端（空殼建立／樣區存檔／示範資料／Excel 匯入精靈）一律硬編 true、對邊界的點面套疊從未實作 → 報表這欄永遠 TRUE、零資訊量且誤導審查者以為全部樣區都在範圍內。改為匯出當下即時套疊：新增 analytics.getPlotBoundaryStatus(plot, project, boundary?) 三態判定（界內／界外／無邊界可比對；另有「樣區無座標」），匯出欄位改名「在專案範圍內」輸出中文三態字串，邊界只 parse 一次逐列共用；關鍵是「專案沒上傳邊界」不得輸出成「界外」——缺資料與資料有問題必須分開。同一判定同步套到樣區明細「在計畫範圍內」（原本 insideBoundary!==false 就顯示 ✅，等於永遠 ✅）。四個寫入端停止寫 insideBoundary 並加註原因（不做 backfill：舊值全 true，資料量與風險不划算，schema 文件已標廢止、讀取端已全數改為即時計算）。judgement 沿用 v2.11.93 的 plot-polygon.js isPointInBoundaryGeoJson（tests/test-boundary-containment.mjs 24/24 通過，未改動該純函式）。?v= 21192→21193。以下歷史 // v2.11.93（地圖界外樣區紅點＋樣區代號 label）：地圖樣區點位改為即時對專案邊界做點面套疊 — 落在邊界外的樣區一律紅點（r=5 加粗邊、蓋過 QA/密度色階）＋樣區面轉紅＋popup 顯示「座標在專案邊界外，請回現場確認 GPS」警語＋legend 固定附紅點說明；原本界外只是灰點（其實是 QA pending 色），野外辨識不出來。判定用新增的 plot-polygon.js isPointInBoundaryGeoJson()（ray casting，支援 FeatureCollection/Feature/Polygon/MultiPolygon＋孔洞環），判定點取樣區記錄座標（plot.location）而非衍生的 SW 錨點；plot 文件上的 insideBoundary 欄位寫入端一律硬編 true 不可信，已不再讀取。另每個樣區點位加代號小字 label（permanent tooltip .plot-code-label，界外轉紅字），並在「疊加圖層」加 🏷️ 樣區代號 checkbox（預設開，密集時可關）。?v= 21191→21192。以下歷史 // v2.11.92（樣區邊界也支援 Shapefile）：承 v2.11.91，樣區「不規則多邊形邊界」上傳同樣吃 ESRI Shapefile（.zip 或多選 .shp/.shx/.dbf/.prj）— 共用 js/shapefile-loader.js，轉出的 GeoJSON 走既有那條路（looksLikeProjectBoundary 防呆 → parseGeoJsonPolygon → local m → VERTEX_MAX 檢查），樣區既有防呆一條未減；Shapefile 解析提醒（編碼偵測/缺 .prj）併入上傳結果訊息一起顯示。以下歷史 // v2.11.91（專案邊界支援 Shapefile）：專案邊界上傳除 GeoJSON 外新增 ESRI Shapefile — 可上傳 .zip 或一次多選 .shp/.shx/.dbf/.prj/.cpg；新模組 js/shapefile-loader.js（lazy 載入 shpjs CDN，不進冷啟動路徑）；有 .prj 依 WKT 投影到 WGS84、無 .prj 交既有 TWD97/WGS84 自動偵測；DBF 中文屬性以候選編碼（.cpg → utf-8 → big5）評分擇優，解決台灣政府圖資常見 Big5 無 .cpg 亂碼；轉出後匯流回既有 parseProjectBoundaryGeoJson()，Leaflet 疊圖/bbox/地籍 dropdown 下游零改動；邊界大小檢查改量輸出字串的 UTF-8 位元組（舊版量原始檔字元數，中文屬性會低估）。以下歷史 // v2.11.90（QA 閉環）：調查者修改「已審核通過(verified)」的自建立木/子項時，自動退回 pending 重新送審 — applySurveyorReQaReset 觸發清單 flagged/rejected → 加 verified；補「已驗證項目被改卻仍掛 verified」的 MRV 可查證性漏洞（存檔 toast 自動顯示「已更新（重新送審）」，PI 待審核會再出現）；專案整案 verified 後 locked，此路徑只在未鎖定+個別項目 verified 時觸發。以下歷史 // v2.11.89（手機更新通知）：修「手機不出現新版本更新資訊」— 加前景版本輪詢：app.js 用 import.meta.url 的 ?v= 為執行版本，比對線上 SW 的 JS_VERSION，不同即跳更新橫幅；於 載入/分頁重新可見(visibilitychange)/每15分 各查一次（補 SW updatefound 在手機背景不觸發的缺口）；更新鈕無 waiting SW 時直接 location.reload()＋3 秒保險重載。以下歷史 // v2.11.88（bugfix×2）：(1) 複查進度徽章顯示條件 !isShell → total>0：研究樣區（土肉桂 location=null 被判空殼）有 30 株 4 期資料卻看不到「待複查 Y/X」徽章，改為有立木即顯示。(2) onAuthStateChanged 包 try/catch：手機重登若 getDoc(users/uid) 網路失敗會中斷 handler、route() 永不執行→停空白回不去；改為即使失敗也一定 renderTopnav()+route() 不卡死。以下歷史 // v2.11.87（I-6c 續）：樣區清單「本期複查進度」— 複查樣區（≥2期）卡片加狀態徽章「⏳ 待複查 Y/X → ✅ 已複查」（Y=本期已複查株數、X=總株數），全株本期已複查即轉綠；沿用清單既有那圈 tree fetch 計數（零額外讀取）；判斷邏輯抽 treeMeasuredInPeriod/plotHasMultiPeriod 與立木清單紅/黑字共用。以下歷史 // v2.11.86（I-6c）：複查立木清單「上期紅字→本期黑字」— 複查期別（≥2期）樣區，尚未本期複查的立木其 DBH/H（仍是上一期值）顯示紅字，本期存檔更新後轉黑；立木存檔加蓋 lastMeasuredPeriod=當期期號（清單精確判斷，回退用 priorSnapshot.stampedAt vs updatedAt）；立木清單頂加「本期複查進度 X/Y 株」+圖例。以下歷史 // v2.11.85（bugfix）：修手機「回不到我的專案、登入也卡住」— 根因為 body-level 全螢幕 overlay（樹種字典 alias/匯入/新增 3 個 + AI 辨識 1 個，皆 document.body.appendChild + fixed inset-0）在 route()/登出時不被清除（route 只清 #app），殘留蓋住全畫面。修：4 個 overlay 標 .app-overlay；route() 開頭一律 closeModal() + 移除所有 .app-overlay。以下歷史 // v2.11.84：複查匯出集中到「匯出」分頁 — 逐棵逐期原始值Excel + 成長摘要CSV(standalone) 從儀表板移至匯出分頁與其他匯出並列；匯出分頁 role 由 pi,reviewer 放寬為 pi,surveyor,reviewer,admin（含 admin god-view，補既有 gating 缺口）；儀表板複查卡片僅留檢視+摘要CSV(view-tied)。以下歷史 // v2.11.83（B）：逐棵逐期原始值匯出 — 儀表板複查報表卡片加「⬇️ 逐棵逐期原始值(Excel)」，讀 trees/{id}/measurements 全期別輸出長格式（一列=一樹一期，含 DBH/樹高/枝下高/活力/病蟲害/斷面積/材積/生物量/碳/CO2/狀態/位置/記錄者/時間）；tree 文件僅存最新快照故第1..N-1期原始值必從逐期 measurement 還原；與 I-6 聚合摘要互補。以下歷史 // v2.11.82：複查期別批次控制 — 樣區清單加「📅 全部樣區開新一期」（PI/admin，各樣區 currentSeq+1，專案解鎖/查證簿只做一次）＋「↩️ 撤銷全部樣區最新一期」（all-or-nothing 資料安全檢查：任一樣區最新期已錄 measurements 即整批擋下；並 pop 批次查證簿還原專案鎖定）＋樣區 banner 加「↩️ 撤銷最新一期」（單樣區、僅該期 0 筆時放行、不動專案鎖定）。rules 不需改（沿用既有 pi/admin 期別寫入權）。（v2.11.81 為 rules-only：同樣區指派 surveyor 可互編立木，無 badge。）以下歷史 // v2.11.80：立木 GPS 手動輸入新增「TWD97 X/Y」選項（J-1b）— 原僅經緯度 lat/lng，加 radio 切換經緯度⇄TWD97；TWD97 走 twd97ToWgs84() 轉回 WGS84 匯流回同一套 apply（存 WGS84 location + manuallyAdjusted=true，下游零改）；radio inline width:auto 覆蓋 .field input{width:100%}；台灣範圍外軟提示「X/Y 沒填反」；?v= 全檔 21179→21180。以下歷史 // v2.11.78：修 v2.11.77 指派 checkbox 顯示 UID 片段（如 pCayLwz5）— 名稱快取 race：loadProject 的 prefetchUserLabels 是 fire-and-forget，樣區清單 render 可能搶先於快取就緒。改在 plot-list onSnapshot handler 內 await prefetchUserLabels(surveyorUids) 後才解析 label（generation guard 仍護）。以下歷史 // v2.11.77：樣區「多人指派」— 卡片指派改 checkbox 多選，資料寫 assignedToUids[]（向後相容鏡像 assignedTo=首位）；surveyor 清單過濾/開期 rules 改陣列成員判定；空殼預設 assignedToUids:[]。以下歷史 // v2.11.76：逐棵立木「歷期測值」明細（I-6b）— 立木清單複查樣區(≥2期)每列加 📊 鈕，點開看該樹 P1..Pn 的 DBH/ΔDBH/H/活力/斷面積/材積/碳逐期值（資料源 trees/{id}/measurements）；v2.11.75：立木表單「同樣區重號」防呆 — 新立木 submit 檢查 treeNum 是否已存在；複查期導向「重測既有立木」、第一期直接擋下改號（修 D3 重號雙重計數）＋座標鄰近(<0.5m)軟提示；v2.11.74：輸入端防呆 — validatePolygon 加 local 座標 ±1000m 硬擋（絕對座標誤填當場退回，附「改用專案邊界」提示）＋立木 GPS 反算 local 爆量改記 null（補 v2.11.73 只修 render 端的根本缺口）；v2.11.73：立木分布圖防壞座標凍結（不規則邊界 extent 加 _sane + drawAxes 格線數硬上限）；v2.11.72：複查野外清單上期值加活立木快照回退；v2.11.71：P5b 開新期蓋印 tree.priorSnapshot + 可列印複查野外清單；以下歷史
...
const SHELL = [
  ...
  `./js/image-compress.js?v=${JS_VERSION}`,
  `./js/plot-polygon.js?v=${JS_VERSION}`,
  // v2.11.91：專案邊界 Shapefile 解析（shpjs 本身走 CDN lazy load，不預快取）
  `./js/shapefile-loader.js?v=${JS_VERSION}`,
