...
// v2.11.44：I-4 跨期 delta 即時警示 + I-5 fate/recruitment（接 I-1 measurements）。openTreeForm
//   複查期（currentPeriod>1）編輯既有立木時：抓最近上期 measurement，顯示上期 DBH/H/活力 +
//   期距年，並隨輸入即時算 ΔDBH/ΔH（通用保守上下界 — Q3 已定：ΔDBH<0 / 年增>5cm/yr /
//   無期距 ΔDBH>15cm / ΔH<−1m → amber 軟警示，永不擋存）。I-5：fate select（存活/本期死亡/
//   失蹤/樹牌脫落，僅複查既有立木）+ recruitment 自動標記（複查期新增→recruitedPeriod）；
//   resurveyFate/recruitedPeriod 寫入 tree 與逐期 measurement 快照；tree 列表加 ✨/☠/❓/🏷️
//   小徽章。純 client + 加性欄位、無 rules / migration。本版 forms.js / app.js 已改 → 全檔
//   ?v= lockstep 21143 -> 21144 + JS_VERSION 21143 -> 21144。⚠ 本輪只 commit 分支、未部署。
const CACHE = 'forest-monitor-v2.11.44';  // v2.11.44：I-4 跨期 delta + I-5 fate/recruitment；以下歷史
